...
class ScanningMicroscopeGUI(tk.Tk):
    def __init__(self):
        ...
        self.image_plot_x = self.ax[0, 0].imshow(self.image_data_x, cmap='bwr', origin='lower')
        self.ax[0, 0].set_title("R map")
        ...
        self.image_plot_y = self.ax[0, 1].imshow(self.image_data_y, cmap='bwr', origin='lower')
        self.ax[0, 1].set_title("deltaR map")
        ...
        self.xstep_button = ttk.Button(button_row_2, text="Set Polarization", command=self.stepX)
    ...
    def stepX(self):
        Ztarget = float(self.xstepEntry.get())
        gotZ = esp.moveZ(Ztarget)
        while gotZ != True:
            time.sleep(2)
            gotZ = esp.moveZ(Ztarget)
    ...
    def update_image_data(self, dataxcur, dataycur,X,Y,datareflection):
        ...
        self.ax[1, 0].cla()
        self.quiver_plot = self.ax[1, 0].quiver(X, Y, dataxcur.transpose(),dataycur.transpose(),
